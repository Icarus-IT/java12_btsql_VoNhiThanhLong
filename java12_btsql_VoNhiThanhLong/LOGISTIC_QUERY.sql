...
SELECT u.USER_ID , u.LOGIN_NAME, u.FIRST_NAME, u.LAST_NAME, u.PHONE, u.EMAIL, u.ADDRESS, u.CITY, u.STATE, u.POSTAL_CODE, u.COUNTRY, u.ROLE, u.PASSWORD, u.IS_DELETED
FROM USER u
WHERE (SELECT COUNT(US.USER_ID)
                         FROM USER_SHIPMENT US
                        WHERE US.USER_ID = u.USER_ID)>0;    
...
SELECT distinct(v.VOYAGE_ID), v.START_DATE, v.END_DATE,
		r.NAME ROUTE_NAME, 
		s.NAME SHIP_NAME, 
		sh.CARGO_CONTENT, sh.TEQ_QUANTITY,sh.TOTAL_COST, sh.STATUS
FROM VOYAGE v, ROUTE r, SHIP s, VOYAGE_SHIPMENT vs, SHIPMENT sh 
WHERE   v.ROUTE_ID = r.ROUTE_ID 
	AND v.SHIP_ID = s.SHIP_ID 
	AND v.VOYAGE_ID=vs.VOYAGE_ID 
	AND vs.SHIPMENT_ID = sh.SHIPMENT_ID ;

## 6 nâng cao: lấy thêm thông tin: cảng khởi hàng và cảng đến

SELECT distinct(v.VOYAGE_ID), v.START_DATE, v.END_DATE, r.NAME  ROUTE_NAME, s.NAME  SHIP_NAME, sh.CARGO_CONTENT, sh.TEQ_QUANTITY,sh.TOTAL_COST, sh.STATUS,ps.NAME  SOURCE_PORT,pd.NAME DEST_PORT
FROM VOYAGE v, ROUTE r, SHIP s, VOYAGE_SHIPMENT vs, SHIPMENT sh, PORT ps, PORT pd
WHERE   v.ROUTE_ID = r.ROUTE_ID 
	AND v.SHIP_ID = s.SHIP_ID 
	AND v.VOYAGE_ID=vs.VOYAGE_ID 
	AND vs.SHIPMENT_ID = sh.SHIPMENT_ID 
	AND r.SOURCE_PORT_ID = ps.PORT_ID 
	AND r.DEST_PORT_ID = pd.PORT_ID ;


## 7. Viết câu truy vấn trả về thông tin khách hàng gồm: Full name, 
##	  địa chỉ (gồm tên đường, thành phố, bang), phone, email,  tên quốc gia
SELECT concat(u.FIRST_NAME,' ',u.LAST_NAME ) FULL_NAME, concat(u.ADDRESS,' ',u.CITY,' ',u.STATE)  ADDRESS, PHONE, EMAIL, c.NAME  COUNTRY
